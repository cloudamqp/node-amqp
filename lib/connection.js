...
Connection.prototype._onMethod = function (channel, method, args) {
  ...
  switch (method) {
    ...
    case methods.connectionTune:
      ...
      if (args.channelMax) {
        if (args.channelMax === 0 || channelMax === 0) {
          channelMax = Math.max(args.channelMax, channelMax);
        } else {
          channelMax = Math.min(args.channelMax, channelMax);
        }
        channelMax = channelMax || 65535;
        debug && debug("tweaking channelMax to " + channelMax);
      }
